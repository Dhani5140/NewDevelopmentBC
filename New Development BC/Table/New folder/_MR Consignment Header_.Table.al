table 80115 "MR Consignment Header"
{
    Caption = 'MR Consignment Header';
    DataClassification = ToBeClassified;
    LookupPageID = "MR Consignment List";
    DrillDownPageId = "MR Consignment List";
    ...
    fields
    {
        field(1; "Material Req. No."; Code[20])
        {
            ...
            begin
                IF "Material Req. No." <> xRec."Material Req. No." THEN BEGIN
                    grecMSISetup.GET;
                    //NoSeriesMgt.TestManual(grecMSISetup."MR Consignment Nos.");
                    NoSeries.TestManual(grecMSISetup."MR Consignment Nos.");
                    "No. Series" := '';
                end;
            END;
        }
        ...
        field(7; "Location Code"; Code[20])
        {
            ...
            trigger OnValidate()
            var
                lrecML: Record "MR Consignment Line";
        ...
        field(20; "Total Amount"; Decimal)
        {
            ...
            CalcFormula = sum("MR Consignment Line"."Line Amount" where("Material Req. No." = field("Material Req. No.")));
        }
    ...
    var
        ...
        gRecMRHeader: Record "MR Consignment Header";
    ...
    begin
        IF "Material Req. No." = '' THEN BEGIN
            grecMSISetup.GET;
            grecMSISetup.TestField("MR Consignment Nos.");
            //NoSeriesMgt.InitSeries(grecMSISetup."MR Consignment Nos.", xRec."No. Series", 0D, "Material Req. No.", "No. Series");
            NoSeries.AreRelated(grecMSISetup."MR Consignment Nos.", xRec."No. Series");
        END;
    ...
    trigger OnDelete()
    var
        lrecMaterialReqLine: Record "MR Consignment Line";
    begin
    ...
    begin
        grecMSISetup.GET;
        EXIT(grecMSISetup."MR Consignment Nos.");
    end;
    ...
    begin
        lRecMSISetup.GET;
        lRecMSISetup.TESTFIELD("MR Consignment Nos.");
        // IF NoSeriesMgt.SelectSeries(lRecMSISetup."MR Consignment Nos.", xRec."No. Series", Rec."No. Series") THEN BEGIN
        //     NoSeriesMgt.SetSeries(Rec."Material Req. No.");
        IF NoSeries.LookupRelatedNoSeries(lRecMSISetup."MR Consignment Nos.", xRec."No. Series", Rec."No. Series") THEN BEGIN
    ...
    begin
        ...
        IF lRecMappingCOA.FINDFIRST THEN BEGIN
            ...
            Rec.VALIDATE("Shortcut Dimension 6 Code", lRecMappingCOA."Shortcut Dimension 6 Code");
            Rec.VALIDATE("Shortcut Dimension 7 Code", lRecMappingCOA."Shortcut Dimension 7 Code");
        ...
        IF lRecMRLine.FIND('-') THEN BEGIN
            REPEAT
                case FieldRef of
                    ...
                    FieldNo("Vendor No."):
                        lRecMRLine.VALIDATE("Vendor No.", "Vendor No.");
                    FieldNo("Document Date"):
                        lRecMRLine.VALIDATE("Document Date", "Document Date");
                    FieldNo("Gen Bus. Posting Group"):
                        lRecMRLine.VALIDATE("Gen Bus. Posting Group", "Gen Bus. Posting Group");
                end;
